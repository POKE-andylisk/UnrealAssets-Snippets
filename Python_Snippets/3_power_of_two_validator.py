# checks the selected textures to see if the image resolution is a power of 2
import unreal
import math
...
for asset in selected_assets:
    ...
    try:
        ...
        if not is_x_valid or not is_y_valid:
            unreal.log("This texture is not power of two: {} ({},{})".format(asset_name, x_size, y_size))
            unreal.log("Asset path: {}".format(asset_path))
            not_pow+=1

    except Exception as err:
        unreal.log_error("{} is not a Texture - {}".format(asset_name, err))
